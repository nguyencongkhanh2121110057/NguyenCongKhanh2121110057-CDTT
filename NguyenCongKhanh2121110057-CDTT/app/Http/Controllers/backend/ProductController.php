...
class ProductController  extends Controller
{
    ...
    {                       
        ...
        $list = Product::join('nck_category', 'nck_product.category_id', '=', 'nck_category.id')
    ->join('nck_brand', 'nck_product.brand_id', '=', 'nck_brand.id')
    ->where('nck_product.status', '<>', '0')
    ->orderBy('nck_product.status', 'desc') 
    ->select("nck_product.*", "nck_category.name as category_name", "nck_brand.name as brand_name")
    ->get();
    ...
    public function edit( $id)
    {
        ...
        $category = Category ::where('status','<>','0')->orderBy('created_at','desc')->get();  
        $brand = Brand ::where('status','<>','0')->orderBy('created_at','desc')->get();  
        $list = Product ::where('status','<>','0')->orderBy('created_at','asc')->get();  
